...
export default function AdminLayout({
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen bg-[#FDFCF9] overflow-x-hidden">
      ...
      {/* Admin Sidebar */}
      <aside className={`fixed md:sticky top-0 left-0 h-screen w-[84%] max-w-xs md:w-72 bg-white border-r border-brand-green/5 flex-shrink-0 soft-shadow z-[50] transition-transform duration-300 ${isSidebarOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"}`}>
        <div className="p-5 md:p-8 h-full flex flex-col">
          <div className="mb-12">
            ...
            <nav className="space-y-3 md:space-y-4">
              <Link href="/admin" onClick={() => setIsSidebarOpen(false)} className="flex items-center gap-4 text-gray-400 hover:text-brand-green bg-transparent hover:bg-brand-green/5 px-4 py-3 md:py-4 rounded-2xl transition-all group font-bold text-sm uppercase tracking-widest">
                <LayoutDashboard className="w-5 h-5 transition-transform group-hover:scale-110" />
                <span>Overview</span>
              </Link>
              <Link href="/admin/products" onClick={() => setIsSidebarOpen(false)} className="flex items-center gap-4 text-gray-400 hover:text-brand-green bg-transparent hover:bg-brand-green/5 px-4 py-3 md:py-4 rounded-2xl transition-all group font-bold text-sm uppercase tracking-widest">
                <ShoppingBasket className="w-5 h-5 transition-transform group-hover:scale-110" />
                <span>Inventory</span>
              </Link>
              <Link href="/admin/orders" onClick={() => setIsSidebarOpen(false)} className="flex items-center gap-4 text-gray-400 hover:text-brand-green bg-transparent hover:bg-brand-green/5 px-4 py-3 md:py-4 rounded-2xl transition-all group font-bold text-sm uppercase tracking-widest">
                <ShoppingBag className="w-5 h-5 transition-transform group-hover:scale-110" />
                <span>Orders</span>
              </Link>
              <Link href="/admin/users" onClick={() => setIsSidebarOpen(false)} className="flex items-center gap-4 text-gray-400 hover:text-brand-green bg-transparent hover:bg-brand-green/5 px-4 py-3 md:py-4 rounded-2xl transition-all group font-bold text-sm uppercase tracking-widest">
      ...
      <div className="flex-grow relative z-10 overflow-y-auto">
        ...
        <div className="absolute top-0 right-0 w-[300px] h-[300px] md:w-[500px] md:h-[500px] bg-brand-green/2 blur-[120px] rounded-full pointer-events-none -z-10"></div>
        <div className="absolute bottom-0 left-0 w-[260px] h-[260px] md:w-[400px] md:h-[400px] bg-brand-light-green/2 blur-[100px] rounded-full pointer-events-none -z-10"></div>
        
        <header className="h-16 md:h-24 bg-white/50 backdrop-blur-md border-b border-brand-green/5 px-4 md:px-8 flex items-center justify-between sticky top-0 z-30">
